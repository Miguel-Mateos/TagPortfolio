...
export const Sidebar: React.FC<ISidebarProps> = (props) => {
  const {id, collapsed, children, defaultItemSelected, className, onClickSidebarItem, ...rest} = props;
  ...
  const renderSidebarLogo = () => {
    let _sidebarLogo = children ? (children as any).find((a: any) => a && a.type === SidebarLogo) : undefined;
    if (_sidebarLogo) {
        ...
        if (child) {
          if (child.type === SidebarDivider) {
            ...
            if (child.props.dropdown) {
              ...
              let _selected = false;
              _childsDropdown?.forEach((_childDropdown: any, _index: number) => {
                ...
                _newChildsDropdown.push(
                  React.cloneElement(_childDropdown, {
                    ...
                    className: `dropdown-item${itemSelected === _childDropdownId ? '_selected' : ''}`,
                    onClick: onClickItem,
                    'data-testid':
                      rest && rest['data-testid'] ? `${rest['data-testid']}-button-${index}-dropdown-${_index}` : undefined,
                  })
                );
              });
              _sidebarChilds.push(
                React.cloneElement(child, {
                  ...
                  collapsed: isCollapsed,
                  'data-testid': rest && rest['data-testid'] ? `${rest['data-testid']}-button-${index}` : undefined,
                })
              );
            } else {
              _sidebarChilds.push(

...
export const Slider: React.FC<ISlider> = (props) => {
  ...
  useEffect(() => {
    if (maxValRef.current) {
      const minPercent = getPercent(0);
      const maxPercent = getPercent(maxVal);

      if (rangeRef.current) {
        rangeRef.current.style.width = `calc(${maxPercent - minPercent}% ${
          maxPercent - minPercent < 50 && maxPercent < 50 ? '+ var(--unit)' : ''
        })`;
      }
    }
  }, [maxVal, getPercent]);
...
const DoubleSlider: React.FC<ISlider> = (props) => {
  const {min = 0, max = 100, onChange, disabled, label, tooltip, tooltipText, defaultValue, className, ...rest} = props;
  const rangeRef = useRef<HTMLDivElement>(null);

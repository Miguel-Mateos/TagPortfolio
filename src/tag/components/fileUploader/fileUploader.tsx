...
export const FileUploader: React.FC<IFileUploaderProps> = (props: IFileUploaderProps) => {
  const {
    small,
    secondary,
    multiple,
  ...
  const renderFiles = () => {
    if (multiple && (valueState as IFile[])?.length > 0) {
      ...
      return _valuesRender.map((_valueRender: IFile, index: number) => (
        <div className="file-uploader-file-container" key={`file-${index}`}>
          ...
          {_valueRender.error && <small>{_valueRender?.error}</small>}
        </div>
      ));
    }
    return (
    ...
    >
      ...
      {dragAndDrop ? (
        ...
        <button
          ...
          className={`${secondary ? 'button-secondary-text-icon_right' : 'button-primary-text-icon_right'}`}
          onClick={onClickShowLoadFromPc}
          disabled={disabled}
        >
          Add files
      ...
      {((!multiple && value) || (multiple && value && (value as IFile[]).length > 0)) && (
        <div className="file-uploader-files-container">{renderFiles()}</div>
      )}
    </div>
  );
};

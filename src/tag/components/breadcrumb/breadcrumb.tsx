    ...
    if (children) {
      ...
      if (_children.length > 1) {
        _children.forEach((_child: any, index: number) => {
          ...
          if (
            ...
            breadcrumbList.push(
              <>
                <div
                  id={_child.props?.title + index + '-breadcrumb'}
                  key={index}
                  data-testid={
                    _child.props && _child.props['data-testid']
                      ? `${_child.props['data-testid']}`
                      : undefined
                  }
                ...
                <span
                  key={`separator-${index}`}
                  className="material-icons breadcrumb-separator"
                >
                  chevron_right
                </span>
              </>
            ...
            _dropdownMenu.push(
              <div
                id={_child.props?.title + '-dropdown' + index}
                key={index}
                data-testid={
                  _child.props && _child.props['data-testid']
                    ? `${_child.props['data-testid']}`
          ...
          if (
            ...
            breadcrumbList.push(
              <div
                id={_child.props?.title + '-breadcrumblist' + index}
                key={index}
                data-testid={
                  _child.props && _child.props['data-testid']
                    ? `${_child.props['data-testid']}`
        ...
        if (_uniqueChild.type === BreadcrumbItem) {
          breadcrumbList = (
            <div
              id={_uniqueChild.props?.title + '-breadcrumblist-unique'}
              data-testid={
                _uniqueChild.props && _uniqueChild.props['data-testid']
                  ? `${_uniqueChild.props['data-testid']}`
                  : undefined
              }
              onClick={() => onClickBreadcrumb}

...
export const Repos: FC<IRepos> = ({ repos }) => {
  ...
  const [isFirstTime, setIsFirstTime] = useState(localStorage.getItem('first-time') === null)
  let timer: any
  ...
  return (
    <>
      <h2 ref={ref} className={`${isFirstTime ? 'first-time' : ''}`}>Repos</h2>
        ...
        {option === 0
          ...
          {repos && (moreRepos ? repos : repos.slice(0, 4)).map((repo, index) => (
            <div key={index + 'repo'} className='card fade-in' style={{ flex: '1 1 20rem', justifyContent: 'center' }}>
                <h3>{repo.name}</h3>
        ...
        <button className='button minimal' onClick={() => setMoreRepos(!moreRepos)}>{moreRepos ? 'See Less...' : 'See More...'}</button>
          ...
          <div className='repos-container-repos'>
            {repos && repos.map((repo, idx) => (
              <div key={idx + 'repo'} style={{ userSelect: 'none' }} onClick={() => repo.name && setMark(repo.name)} className='card repo interactive'>
                <h3>{repo.name}</h3>
              </div>
            ))}
          </div>
          {mark
            ? <div className='card repos-container-mark'><Markdown repo={mark}/></div>
